...
- Fixed `test_outbox_concurrency` failures on SQLite by improved retry logic in `Dispatcher` (`_fetch_candidates` and `_safe_save` for status updates) and robust worker test loops.
- Removed `.DS_Store` file which causes CI failures.
