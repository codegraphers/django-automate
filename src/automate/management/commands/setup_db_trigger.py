...
class Command(BaseCommand):
    ...
    def add_arguments(self, parser):
        parser.add_argument("table", nargs="?", type=str, help="Table name to add trigger to")
        parser.add_argument("--list", action="store_true", help="List all tables with automate triggers")
        parser.add_argument("--remove", action="store_true", help="Remove trigger from table instead of adding")
        parser.add_argument("--all-models", action="store_true", help="Add triggers to all Django model tables")

    def handle(self, *args, **options):
        db_engine = connection.settings_dict.get("ENGINE", "")
        if "postgresql" not in db_engine and "psycopg" not in db_engine:
            self.stdout.write(self.style.ERROR(f"❌ This command requires PostgreSQL. Current: {db_engine}"))
            return
    ...
    def _add_trigger(self, table: str):
        ...
        with connection.cursor() as cursor:
            ...
            self.stdout.write(self.style.SUCCESS(f"✅ Trigger added to table: {table}"))
    ...
    def _remove_trigger(self, table: str):
        ...
        with connection.cursor() as cursor:
            ...
            self.stdout.write(self.style.SUCCESS(f"✅ Trigger removed from table: {table}"))
