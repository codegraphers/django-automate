...
from automate_core.workflows.models import Automation, Trigger, Workflow
...
class Command(BaseCommand):
    ...
    def handle(self, *args, **options):
        ...
        wf, _ = Workflow.objects.get_or_create(
            automation=auto, version=1, defaults={"graph": {"nodes": [{"id": "step1", "type": "log"}]}}
        )
        trigger, _ = Trigger.objects.get_or_create(
            automation=auto, is_active=True, type="manual", event_type="manual.test"
        )
        event = Event.objects.create(
            tenant_id=tenant, event_type="manual.test", source="cli", payload={}, occurred_at=timezone.now()
        )
