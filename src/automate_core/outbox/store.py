...
class SkipLockedClaimOutboxStore(OutboxStore):
    ...
    def claim_batch(self, owner: str, limit: int, now: datetime) -> list[OutboxItem]:
        ...
        with transaction.atomic():
            ...
            qs = OutboxItem.objects.select_for_update(skip_locked=True).filter(
    ...
    def mark_done(self, item_id: int, owner: str) -> None:
        OutboxItem.objects.filter(id=item_id, lease_owner=owner).update(status="DONE", lease_owner=None, lease_expires_at=None)
...
class OptimisticLeaseOutboxStore(OutboxStore):
    ...
    def claim_batch(self, owner: str, limit: int, now: datetime) -> list[OutboxItem]:
        ...
        candidates = list(OutboxItem.objects.filter(
            status__in=["PENDING", "RETRY"],
            next_attempt_at__lte=now,
            lease_owner__isnull=True  # Simplified check
        ).values_list("id", flat=True).order_by("priority", "next_attempt_at")[:limit])
