...
class LeaseManager:
    ...
    def acquire_execution(self, execution_id: str, ttl_seconds: int = 60) -> bool:
        ...
        updated = (
            ...
            .update(lease_owner=self.worker_id, lease_expires_at=expires_at, heartbeat_at=now)
        )
        ...
        updated_fresh = Execution.objects.filter(id=execution_id, lease_owner__isnull=True).update(
            lease_owner=self.worker_id,
    ...
    def heartbeat_execution(self, execution_id: str, ttl_seconds: int = 60) -> bool:
        ...
        updated = Execution.objects.filter(id=execution_id, lease_owner=self.worker_id).update(
            lease_expires_at=expires_at, heartbeat_at=now
        )
        return updated > 0

    def release_execution(self, execution_id: str):
        Execution.objects.filter(id=execution_id, lease_owner=self.worker_id).update(
