...
class KnowledgeSource(models.Model):
    ...
    def test_connection(self):
        ...
        try:
            provider = get_retrieval_provider(self.provider_key)
...
class RAGEndpoint(models.Model):
    ...
    rate_limit = models.IntegerField(
        default=100,
        help_text="Max queries per minute"
    )
...
class EmbeddingModel(models.Model):
    ...
    is_default = models.BooleanField(
        default=False,
        help_text="Use this model if none specified"
    )
...
class RAGQueryLog(models.Model):
    ...
    query_hash = models.CharField(
        max_length=64,
        help_text="SHA256 hash of query (for privacy)"
    )

    latency_ms = models.IntegerField(help_text="Total query latency in ms")

    results_meta = models.JSONField(
        default=dict,
        help_text="Metadata: doc_ids, scores, count"
    )
