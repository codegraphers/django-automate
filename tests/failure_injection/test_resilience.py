...
from automate.models import Execution, ExecutionStatusChoices, ExecutionStep
...
class TestFailureInjection:
    ...
    def test_worker_crash_mid_step(self, db):
        ...
        with patch("automate.registry.registry.get_connector") as mock_get:
            ...
            # Create Execution
            execution = Execution.objects.create(status=ExecutionStatusChoices.RUNNING)
            ...
            # Verify State
            step = ExecutionStep.objects.get(step_id="step_crash")
            assert step.status == ExecutionStatusChoices.FAILED
            assert "Worker Died!" in step.error_message
    ...
    def test_retry_storm_control(self, db):
        ...
        """
        runtime = Runtime()
        execution = Execution.objects.create(status=ExecutionStatusChoices.RUNNING, max_retries=2)
